...
def spatial_transformer_network(input_fmap, theta, out_dims=None, **kwargs):
    ...
    B = tf.shape(input_fmap)[0] #batch size
    ...
    theta = tf.reshape(theta, [B, -1])
    #print(tf.keras.backend.int_shape(theta))
...
def affine_grid_generator(height, width, theta):
    ...
    x = tf.linspace(-1.0, 1.0, width) # last dim is number of intervals
