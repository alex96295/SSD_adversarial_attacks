...
patch_results_obj_ale_yolov2 = bb.io.load('det_coco', "C:/Users/Alessandro/Desktop/GitLab/yolov2_pytorch/json_files/ens_maxmax.json", class_label_map={0: 'person'})
patch_results_obj_ale_yolov3 = bb.io.load('det_coco', "C:/Users/Alessandro/Desktop/GitLab/yolov3_pytorch/json_files/ens_maxmax.json", class_label_map={0: 'person'})
patch_results_obj_ale_yolov4 = bb.io.load('det_coco', "C:/Users/Alessandro/Desktop/GitLab/YOLOv4_d2p_adv_COCO/json_files/ens_maxmax.json", class_label_map={0: 'person'})
...
plt.plot(obj_ale_yolov2['recall'], obj_ale_yolov2['precision'], label=f'YOLOV2 - AP_PATCH: {round(ap*100, 2)}%, AP_CLEAN: {88.42}%') #, RECALL: {round(obj_ale_yolov2["recall"].iloc[-1]*100, 2)}%')

ap = bb.stat.ap(obj_ale_yolov3)
plt.plot(obj_ale_yolov3['recall'], obj_ale_yolov3['precision'], label=f'YOLOV3 - AP_PATCH: {round(ap*100, 2)}%, AP_CLEAN: {96.62}%') #, RECALL: {round(obj_ale_yolov3["recall"].iloc[-1]*100, 2)}%')

ap = bb.stat.ap(obj_ale_yolov4)
plt.plot(obj_ale_yolov4['recall'], obj_ale_yolov4['precision'], label=f'YOLOV4 - AP_PATCH: {round(ap*100, 2)}%, AP_CLEAN: {93.53}%') #, RECALL: {round(obj_ale_yolov3_tiny["recall"].iloc[-1]*100, 2)}%')


plt.gcf().suptitle('PATCH:YV2+YV3+YV4 MAXIMUM STRATEGY, dataset = INRIA')
plt.gca().set_ylabel('Precision')
plt.gca().set_xlabel('Recall')
plt.gca().set_xlim([0, 1.05])
plt.gca().set_ylim([0, 1.05])
plt.gca().legend(loc=4)
plt.savefig("C:/Users/Alessandro/PycharmProjects/tf_model_garden/research/object_detection/pr_curves/networks_bunch_vs_single_patch/ens_max/pr_curve_ensemble_max.eps", dpi=500)
plt.savefig("C:/Users/Alessandro/PycharmProjects/tf_model_garden/research/object_detection/pr_curves/networks_bunch_vs_single_patch/ens_max/pr_curve_ensemble_max.png", dpi=500)
