...
def visualize_boxes_and_labels_on_image_array(
  ...
  box_to_display_str_map_out = collections.defaultdict(list)
      ...
      if scores is None:
        ...
        if not skip_labels:
          if not agnostic_mode:
            ...
            display_str_out = category_index[classes[i]]['id'] - 1
        if not skip_scores:
          if not display_str:
            display_str = '{}%'.format(int(100*scores[i]))
            display_str_out = '{}'.format(scores[i])
            scores_float.append(scores[i])
          else:
            display_str = '{}: {}%'.format(display_str, int(100*scores[i]))
            display_str_out = [display_str_out, scores[i]]
            scores_float.append(scores[i])
        if not skip_track_ids and track_ids is not None:
          if not display_str:
            display_str = 'ID {}'.format(track_ids[i])
            display_str_out = 'ID {}'.format(track_ids[i])
          else:
            display_str = '{}: ID {}'.format(display_str, track_ids[i])
            display_str_out = [display_str_out, track_ids[i]]
        box_to_display_str_map[box].append(display_str)
        box_to_display_str_map_out[box].append(display_str_out)
        if agnostic_mode:
  ...
  for box, color in box_to_color_map.items():
    ...
    if keypoints is not None:
      draw_keypoints_on_image_array(
          ...
          use_normalized_coordinates=use_normalized_coordinates)
    #print(box_to_display_str_map_out)

  return image, box_to_display_str_map_out
