...
def pad_and_scale(model_image_size, img, lab):
    ...
    if w == h:
        ...
        if dim_to_pad == 1:
            ...
            l_tensors = [tfk.backend.expand_dims(t) for t in [l0, l1, l2, l3, l4, l5]]
...
def patch_and_show_inference(model, image_path, label, label_nopad):
    # the array based representation of the image will be used later in order to prepare the
    # result image with boxes and labels on it.
    image = Image.open(image_path)
    image_dims = image.size

    image_pad, label_p = pad_and_scale(model_image_size, image, label)
    ...
    lab_fake_batch_nopad = tfk.backend.expand_dims(label_nopad, 0)

    print('Clean label original: ' + str(tfk.backend.int_shape(lab_fake_batch)))

    lab_fake_batch_boxsplit = tf.unstack(lab_fake_batch, axis=1) # to get each box line separated for patch application
    lab_fake_batch_boxsplit_nopad = tf.unstack(lab_fake_batch_nopad, axis=1)

    single_img_labdir_c = os.path.join(savedir, name, 'clean/')
    ...
    for single_box_label, single_box_label_nopad in zip(lab_fake_batch_boxsplit, lab_fake_batch_boxsplit_nopad):
        print(tfk.backend.int_shape(single_box_label))
        #single_box_label_sq = tf.squeeze(single_box_label,0)

        single_box_clean_name = name + '_box' + str(k) + '-c.txt'
        single_box_clean_path = os.path.join(single_img_labdir_c, single_box_clean_name)

        textfile_clean = open(single_box_clean_path, 'w+')
        textfile_clean.write(f'{single_box_label_nopad[0][0]} {single_box_label_nopad[0][1]} {single_box_label_nopad[0][2]} {single_box_label_nopad[0][3]} {single_box_label_nopad[0][4]} {single_box_label_nopad[0][5]}')

        single_box_label = tf.expand_dims(single_box_label, axis=1)

        box_name = name + '_box' + str(k) + '.jpg'
        k+=1
        adv_batch_t = patch_transformer.forward(adv_patch, single_box_label, model_image_size, do_rotate=True, rand_loc=False)
        ...
        p_img_orig.save(os.path.join("C:/Users/Alessandro/Desktop/single_box_trial_ssdlite_mbntv2_coco_random/patch_applied_images_no_det/", box_name))
        ...
        im_out_pil = tfk.preprocessing.image.array_to_img(im_out)
        im_out_pil.save(os.path.join("C:/Users/Alessandro/Desktop/single_box_trial_ssdlite_mbntv2_coco_random/patch_applied_images_det/", box_name))

        im_outperbox_list.append(im_out)
        dict_imageperbox_list.append(dict)

    return im_outperbox_list, dict_imageperbox_list, image_dims
    #display(Image.fromarray(image_np))


# In[ ]:

savedir = "C:/Users/Alessandro/Desktop/single_box_trial_ssdlite_mbntv2_coco_random/ssd_lite_mnv2_patched_fewdata_labels/"
patchfile = "C:/Users/Alessandro/Desktop/GitLab/YOLOv2_d2p_adv_COCO_thys2019/saved_patches_mytrial/ssdlitev2_ok/mbntv2_ssdlite_max_objcls_1000epochs_patchsize300_meanstd.jpg"
txtpath_c = "C:/Users/Alessandro/Desktop/single_box_trial_ssdlite_mbntv2_coco_random/ssd_lite_mnv2_clean_fewdata-labels/"
...
for image_path in TEST_IMAGE_PATHS:
    ...
    if image_path_str.endswith('.jpg') or image_path_str.endswith('.png'):
        ...
        single_img_labdir = os.path.join(savedir, name, 'patch/')

        if os.path.isdir(single_img_labdir) == False:
            os.makedirs(single_img_labdir)

        # txtpath_p = os.path.join(savedir, 'proper_patched_obj/', 'yolov3-labels/', txtname)
    ...
    label_nopad = tf.convert_to_tensor(label_nopad, dtype=tf.float32)
    # print(tfk.backend.int_shape(label))

    if tfk.backend.ndim(label) == 1:
        # label = label.unsqueeze(0)
        label = tfk.backend.expand_dims(label, 0)
        label_nopad = tfk.backend.expand_dims(label_nopad, 0)
    print('label file used for patch application read correctly')

    im_out_list_perbox, dict_list_perbox, image_dims = patch_and_show_inference(detection_model, image_path, label, label_nopad)

    for i, dict in enumerate(dict_list_perbox):
        box_name = name + '_box' + str(i+1) + '.txt'

        txtpath_p = os.path.join(single_img_labdir, box_name)
        im_out = Image.fromarray(im_out_list_perbox[i])
